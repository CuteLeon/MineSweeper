Public Class MineSweeperForm
    ...
    Dim CloseBitmap As Bitmap = My.Resources.MineSweeperAssets.Close_N '初始关闭按钮
    Dim CloseRectangle As New Rectangle(308, 1, CloseBitmap.Width, CloseBitmap.Height) '关闭按钮的位置
    ...
    Private Sub MineSweeperForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        '设置窗口图标
        Me.Icon = My.Resources.MineSweeperAssets.Mine_Sweeper
        '设置鼠标样式
        Dim CursorBitmap As New Bitmap(My.Resources.MineSweeperAssets.TheCursor)
        Dim CursorHandle As IntPtr = CursorBitmap.GetHicon()
        Dim TheCursor = New Cursor(CursorHandle)
        Me.Cursor = TheCursor
        MinefieldPanel.Cursor = TheCursor
        '释放内存
    ...
    Private Sub MineSweeperForm_MouseMove(sender As Object, e As MouseEventArgs) Handles Me.MouseMove
        ...
        If IsMouseIn Then
            '鼠标在关闭按钮
            CloseBitmap = My.Resources.MineSweeperAssets.Close_E
            CloseState = True
        Else
            '鼠标不在关闭按钮
            CloseBitmap = My.Resources.MineSweeperAssets.Close_N
            CloseState = False
        End If
        '绘制关闭按钮和雷区
        BackgroundGraphics.DrawImage(CloseBitmap, CloseRectangle)
    ...
    Private Sub MineSweeperForm_MouseDown(sender As Object, e As MouseEventArgs) Handles Me.MouseDown
        If CloseRectangle.Contains(e.X, e.Y) Then
            ...
            BackgroundGraphics = Graphics.FromImage(BackgroundBitmap)
            '设置关闭按钮
            CloseBitmap = My.Resources.MineSweeperAssets.Close_D
            '绘制关闭按钮和雷区
            BackgroundGraphics.DrawImage(CloseBitmap, CloseRectangle)
            BackgroundGraphics.DrawImage(MinefieldBitmap, MinefieldRectangle)
            '应用到窗口并释放背景内存
    ...
    Private Sub MineSweeperForm_MouseUp(sender As Object, e As MouseEventArgs) Handles Me.MouseUp
        ...
        BackgroundGraphics = Graphics.FromImage(BackgroundBitmap)
        '设置关闭按钮
        CloseBitmap = IIf(CloseRectangle.Contains(e.X, e.Y), My.Resources.MineSweeperAssets.Close_E, My.Resources.MineSweeperAssets.Close_N)
        '绘制关闭按钮和雷区
        BackgroundGraphics.DrawImage(CloseBitmap, CloseRectangle)
        BackgroundGraphics.DrawImage(MinefieldBitmap, MinefieldRectangle)
        '应用到窗口并释放背景内存
    ...
    Private Sub MineSweeperForm_MouseClick(sender As Object, e As MouseEventArgs) Handles Me.MouseClick
        '点击关闭按钮，退出程序
        If CloseRectangle.Contains(e.X, e.Y) Then Application.Exit()
    End Sub
